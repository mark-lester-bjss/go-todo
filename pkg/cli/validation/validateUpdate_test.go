...
func TestValidateUpdate(t *testing.T) {
	...
	tests := []test{
		{
			input: []string{"mark", "3f296dbb-600c-4240-89fd-6e642bbb841b:todo"},
			wantA: true,
			wantB: ""},
		{
			input: []string{"mark"},
		...
		{
			input: []string{"mark", "3f296dbb-600c-4240-89fd-6e642bbb841btodo"},
			wantA: false,
			wantB: "The args should be a uuid and todo separated by a colon, uuid:todo"},
		{
			input: []string{"mark", "not a uuid:todo"},
			wantA: false,
			wantB: "The uuid supplied is not valid. Type help for more details"},
	}

	for _, tc := range tests {
		valid, message := ValidateUpdate(tc.input)
